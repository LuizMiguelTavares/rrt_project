...
//     int desiredGridWidth = 300; // Desired number of cells horizontally
//     int desiredGridHeight = 300; // Desired number of cells vertically
...
//     // Output the adjusted grid dimensions
//     std::cout << "Adjusted Grid Width (Cells): " << gridWidth << std::endl;
//     std::cout << "Adjusted Grid Height (Cells): " << gridHeight << std::endl;
...
//                 obstacleCells.push_back(cv::Point(x, y));
//             }
//         }
//     }

//     // Determine the grid cell for the starting and goal points
//     cv::Point gridStartingPoint(static_cast<int>(startingPoint.x / cellWidth), static_cast<int>(startingPoint.y / cellHeight));
//     cv::Point gridGoalPoint(static_cast<int>(goalPoint.x / cellWidth), static_cast<int>(goalPoint.y / cellHeight));

//     // Output the results
//     std::cout << "Starting Grid Cell: (" << gridStartingPoint.x << ", " << gridStartingPoint.y << ")" << std::endl;
//     std::cout << "Goal Grid Cell: (" << gridGoalPoint.x << ", " << gridGoalPoint.y << ")" << std::endl;
//     std::cout << "Number of Obstacle Cells: " << obstacleCells.size() << std::endl;

//     // Create a new blank image for the grid visualization
...
//             if(std::find(obstacleCells.begin(), obstacleCells.end(), cv::Point(x, y)) != obstacleCells.end()) {
...
//     cv::circle(gridImage, cv::Point(static_cast<int>(startingPoint.x), static_cast<int>(startingPoint.y)), 5, cv::Scalar(0, 255, 0), cv::FILLED);
//     // Draw the goal point as a red circle
//     cv::circle(gridImage, cv::Point(static_cast<int>(goalPoint.x), static_cast<int>(goalPoint.y)), 5, cv::Scalar(0, 0, 255), cv::FILLED);
