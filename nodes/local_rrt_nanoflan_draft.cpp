            ...
            if (!map_.data.empty() && !path_.poses.empty()) {
                if (!quad_tree_initialized_) {
                    ...
                    if (global_map_.header.frame_id == path_.header.frame_id) {
                        // int index = 0;
                        ROS_INFO("Length of the path: %lu", path_.poses.size());
                        for (const auto& pose : path_.poses) {
                            float x = pose.pose.position.x - global_map_.info.origin.position.x;
                            float y = pose.pose.position.y - global_map_.info.origin.position.y;
                        ...
                        kd_tree = std::make_unique<my_kd_tree_t>(2 /*dim*/, base_cloud_, nanoflann::KDTreeSingleIndexAdaptorParams(10 /* max leaf */));
                        kd_tree->buildIndex();
...
private:   
    ...
    void generateLocalRRT() {
        ...
        const float query_pt[2] = { robot_x, robot_y };

        float map_size = map_.info.width * map_.info.resolution;
        float search_radius = std::pow(map_size * std::sqrt(2) / 2, 2); 

        std::vector<nanoflann::ResultItem<unsigned int, float>> ret_matches; // Pairs of index and squared distance

        // Perform radius search
        size_t n_matches = kd_tree->radiusSearch(&query_pt[0], search_radius, ret_matches, nanoflann::SearchParameters());

        if (ret_matches.empty()) {
            ROS_ERROR("No path point found in the local map.");
            return;
        }

        std::sort(ret_matches.begin(), ret_matches.end(), [](const nanoflann::ResultItem<unsigned int, float>& a, const nanoflann::ResultItem<unsigned int, float>& b) {
        ...
        // kd_tree->findNeighbors(resultSet, query_pt, nanoflann::SearchParameters());

        // std::shared_ptr<motion_planning::Node> closest_path_point = quad_tree_->nearest_neighbor(robot_position);
        // size_t closest_index = ret_index[0];
