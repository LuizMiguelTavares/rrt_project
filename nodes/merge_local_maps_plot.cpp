...
public:
    LocalMapGenerator(ros::NodeHandle& nh)
        ...
        local_map_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(local_map_topic_ + "_plot", 1);
        local_gmapping_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(local_map_topic_ + "_gmapping", 1);
        local_lidar_map_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(local_map_topic_ + "_lidar", 1);
    ...
    void publishLocalMaps(const ros::TimerEvent&) {
        if (!global_map_1_.data.empty()) {
            ...
            local_map_1_.header.stamp = ros::Time::now();
            local_gmapping_pub_.publish(local_map_1_);
        }
        if (!global_map_2_.data.empty()) {
            if(!createLocalMap(global_map_2_, local_map_2_)){
                return;
            }
            local_map_2_.header.stamp = ros::Time::now();
            local_lidar_map_pub_.publish(local_map_2_);
        }
...
private:
    ...
    ros::Publisher local_gmapping_pub_;
    ros::Publisher local_lidar_map_pub_;
