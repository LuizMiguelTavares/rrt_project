...
public:
    ObstacleAvoidance() : latest_scan(nullptr), running(true), max_filtered_points_(0), max_matched_base_points_(0) {
        ...
        filtered_closest_points_pub = nh.advertise<visualization_msgs::MarkerArray>("/filtered_closest_points", 10);
        base_points_pub = nh.advertise<visualization_msgs::MarkerArray>("/base_points", 10);
        matched_base_points_pub = nh.advertise<visualization_msgs::MarkerArray>("/matched_base_points", 10);
        ...
        for (int i = 0; i < num_clusters_; i++) {
            ...
            for (int j = start_index; j != end_index; j = (j + 1) % n_points) {
                ...
                if (point_range < msg->range_max && point_range > msg->range_min) {
                    ...
                    cluster_point_marker.header.frame_id = msg->header.frame_id;
    ...
    void filterClosestPoints(const std::string& frame_id) {
        ...
        // Construct a kd-tree index for closest points
        typedef nanoflann::KDTreeSingleIndexAdaptor<
            nanoflann::L2_Simple_Adaptor<float, PointCloud>,
            PointCloud,
            2 /* dimension */
        ...
        visualization_msgs::MarkerArray matched_base_points_marker_array;
        ...
        for (size_t i = 0; i < closest_points.size(); ++i) {
            ...
            // Add the nearest base point to the matched base points marker array
            size_t nearest_base_point_idx = ret_index[0];
            const auto& base_pt = base_cloud_.pts[nearest_base_point_idx];
            visualization_msgs::Marker matched_base_point_marker;
            matched_base_point_marker.header.frame_id = frame_id;
            matched_base_point_marker.header.stamp = ros::Time::now();
            matched_base_point_marker.ns = "matched_base_points";
            matched_base_point_marker.id = matched_base_points_marker_array.markers.size();
            matched_base_point_marker.type = visualization_msgs::Marker::SPHERE;
            matched_base_point_marker.action = visualization_msgs::Marker::ADD;
            matched_base_point_marker.pose.position.x = base_pt.x;
            matched_base_point_marker.pose.position.y = base_pt.y;
            matched_base_point_marker.pose.position.z = 0;
            matched_base_point_marker.pose.orientation.x = 0.0;
            matched_base_point_marker.pose.orientation.y = 0.0;
            matched_base_point_marker.pose.orientation.z = 0.0;
            matched_base_point_marker.pose.orientation.w = 1.0;
            matched_base_point_marker.scale.x = 0.1;
            matched_base_point_marker.scale.y = 0.1;
            matched_base_point_marker.scale.z = 0.1;
            matched_base_point_marker.color.a = 1.0;  // Alpha
            matched_base_point_marker.color.r = 1.0;
            matched_base_point_marker.color.g = 0.0;
            matched_base_point_marker.color.b = 0.0;

            matched_base_points_marker_array.markers.push_back(matched_base_point_marker);
        }
        ...
        // Update maximum marker count
        if (cleaned_points.size() > max_filtered_points_) {
            max_filtered_points_ = cleaned_points.size();
        }

        // Add delete markers for remaining markers
        for (size_t i = cleaned_points.size(); i < max_filtered_points_; ++i) {
            visualization_msgs::Marker delete_marker;
            delete_marker.header.frame_id = frame_id;
            delete_marker.header.stamp = ros::Time::now();
            delete_marker.ns = "cleaned_closest_points";
            delete_marker.id = i;
            delete_marker.action = visualization_msgs::Marker::DELETE;
            cleaned_points_marker_array.markers.push_back(delete_marker);
        }

        // Publish the cleaned closest points
        filtered_closest_points_pub.publish(cleaned_points_marker_array);

        // Update maximum marker count
        if (matched_base_points_marker_array.markers.size() > max_matched_base_points_) {
            max_matched_base_points_ = matched_base_points_marker_array.markers.size();
        }

        // Add delete markers for remaining markers
        for (size_t i = matched_base_points_marker_array.markers.size(); i < max_matched_base_points_; ++i) {
            visualization_msgs::Marker delete_marker;
            delete_marker.header.frame_id = frame_id;
            delete_marker.header.stamp = ros::Time::now();
            delete_marker.ns = "matched_base_points";
            delete_marker.id = i;
            delete_marker.action = visualization_msgs::Marker::DELETE;
            matched_base_points_marker_array.markers.push_back(delete_marker);
        }

        // Publish the matched base points
        matched_base_points_pub.publish(matched_base_points_marker_array);
    }
